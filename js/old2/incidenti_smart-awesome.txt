...
const basemapStyles = {
    'carto-dark': {
	   ...
	   sources: {
            'carto': {
                ...
                tileSize: 256,
                attribution: '© CARTO | <a href="https://www.dati.gov.it/view-dataset?Cerca=incidenti+palermo" target="_blank">Fonte Dati: dati.gov.it</a> - Rielaborazione: <a href="https://www.linkedin.com/in/gbvitrano/" target="_blank">@gbvitrano</a> - <a href="http://activedatasicilia.it/" target="_blank">activedatasicilia.it</a>'
            }
        },
        layers: [{
            id: 'carto',
    ...
    'osm': {
        ...
        sources: {
            'osm': {
                type: 'raster',
                tiles: ['https://tile.activestreetmap.org/{z}/{x}/{y}.png'],
                tileSize: 256,
                attribution: '© activeStreetMap contributors | <a href="https://www.dati.gov.it/view-dataset?Cerca=incidenti+palermo" target="_blank">Fonte Dati: dati.gov.it</a> - Rielaborazione: <a href="https://www.linkedin.com/in/gbvitrano/" target="_blank">@gbvitrano</a> - <a href="http://activedatasicilia.it/" target="_blank">activedatasicilia.it</a>'
            }
        },
        layers: [{
            id: 'osm',
    ...
    'satellite': {
        ...
        sources: {
            'satellite': {
                ...
                tileSize: 256,
                attribution: '© Google | <a href="https://www.dati.gov.it/view-dataset?Cerca=incidenti+palermo" target="_blank">Fonte Dati: dati.gov.it</a> - Rielaborazione: <a href="https://www.linkedin.com/in/gbvitrano/" target="_blank">@gbvitrano</a> - <a href="http://activedatasicilia.it/" target="_blank">activedatasicilia.it</a>'
            }
        },
        layers: [{
            id: 'satellite',
...
function toggleClustering() {
    activeClustering = !activeClustering;
...
function toggleDesktopSidebar() {
    ...
    localStorage.setItem('sidebarDesktopState', 
        sidebar.classList.contains('collapsed') ? 'collapsed' : 'active'
    );
}
...
// active Detail Panel (AGGIORNATO CON FONT AWESOME)
function activeDetailPanel(properties) {
...
// active 2019 Info Popup (ICONA AGGIORNATA)
function active2019InfoPopup() {
...
// active Top Luoghi Modal (ICONA AGGIORNATA)
function activeTopLuoghiModal() {
...
function activeAnalytics() {
    ...
    if (panel) {
        panel.classList.add('active');
        document.body.classList.add('analytics-panel-active'); // Aggiungi classe al body
        document.body.classList.add('modal-active'); // Aggiungi classe generica
...
function closeAnalytics() {
    ...
    if (panel) {
        ...
        document.body.classList.remove('analytics-panel-active'); // Rimuovi classe dal body
        document.body.classList.remove('modal-active'); // Rimuovi classe generica
    }
}
...
async function downloadChartAsPNG(chartId, filename) {
    ...
    try {
        ...
        if (chartId.includes('serie-storica')) {
            ctx.fillText('Fonte: Istat - ACI - Rielaborazione: activedatasicilia.it', 40, targetHeight - 40);
        } else {
            ctx.fillText('Fonte: dati.gov.it - Rielaborazione: activedatasicilia.it', 40, targetHeight - 40);
        }
        
        ctx.fillText('https://activedatasicilia.github.io/incidenti_palermo/', 40, targetHeight - 20);
...
function initMap() {
    ...
    class ODSControl {
        onAdd(map) {
            ...
            const link = document.createElement('a');
            link.href = 'https://palermohub.activedatasicilia.it/';
            link.target = '_blank';
            link.rel = 'noactiveer noreferrer';
            link.title = 'palermohub.activedatasicilia.it';
            
            const img = document.createElement('img');
            img.src = 'img/pa_hub_new.png';
            img.alt = 'palermohub.activedatasicilia.it';
...
paint: {
    ...
    map.addLayer({
        ...
        layout: {
            ...
            'text-font': ['active Sans Bold', 'Titillium Web Unicode MS Bold'],
            'text-size': [
                'interpolate',
                ['linear'],
                ['get', 'point_count'],
...
// Update Top Luoghi Modal if active
function updateTopLuoghiModalIfactive() {
    const modal = document.getElementById('top-luoghi-modal');
    if (modal && modal.classList.contains('active')) {
        activeTopLuoghiModal();
    }
}
...
function setupEventListeners() {
    ...
    const buttonsConfig = [
        ...
        { id: 'btn-data-table', handler: activeDataTable },
        { id: 'btn-data-table-map', handler: activeDataTable },
        { id: 'btn-analytics', handler: activeAnalytics },
        { id: 'btn-analytics-map', handler: activeAnalytics }
    ];
    ...
    if (btnTopLuoghi) addTouchClickListener(btnTopLuoghi, activeTopLuoghiModal);
